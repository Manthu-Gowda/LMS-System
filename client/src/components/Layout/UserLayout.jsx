...
const Logo = ({ collapsed }) => (
  <div className="flex items-center justify-center text-white h-16 bg-transparent">
    ...
    {!collapsed && <span className="ml-3 text-xl font-semibold">LMS</span>}
  </div>
);
...
const UserLayout = ({ children }) => {
  ...
  const menuItems = [
    ...
    { key: '/certificates', icon: <SafetyCertificateOutlined />, label: <Link to="/certificates">Certificates</Link> },
  ];

  // Updated userMenu to be an object for the `menu` prop
  const userMenu = {
    items: [
      {
        key: 'profile',
