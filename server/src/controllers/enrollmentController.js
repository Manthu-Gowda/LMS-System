...
exports.enrollInCourse = async (req, res) => {
  try {
    ...
    if (!course || !course.isPublished) {
      return res.status(404).json({ statusCode: 404, message: 'Course not found or not available' });
    }

    const existingEnrollment = await Enrollment.findOne({ user: userId, course: courseId });
    if (existingEnrollment) {
      return res.status(400).json({ statusCode: 400, message: 'You are already enrolled in this course' });
    }
