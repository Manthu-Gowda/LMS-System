...
exports.getCertificate = async (req, res) => {
  try {
    ...
    if (!certificate) {
      return res.status(404).json({ statusCode: 404, message: 'Certificate not found' });
    }
    ...
    try {
      ...
      return res.status(404).json({ statusCode: 404, message: 'Certificate file not found' });
    }
    ...
    // Send error as JSON, but don't try to set headers if they are already sent
    if (!res.headersSent) {
      res.status(500).json({ statusCode: 500, message: 'Internal server error' });
    }
  }
